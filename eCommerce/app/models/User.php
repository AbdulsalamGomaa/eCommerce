...
class User extends config implements operations {
    ...
    public function update() {
        ...
        // Update user information in database and in user profile page
        $image = null;
        if($this->image) {
    ...
    function verifyOfCode() {
        
        // check if code is the same code for the same person in database
        $query = "SELECT `email` FROM `users` WHERE `email` = '$this->email' AND `code` = $this->code";
        return $this->runDQL($query);
    }

    function updateOldCode() {
        
        // Update the code in database for the user who didn't verify his account in the first time
        $query = "UPDATE `users` SET code = $this->code WHERE email = '$this->email'";
        return $this->runDML($query);
    }

    function makeUserVerified() {
        
        // Update user status and email verified at
        $query = "UPDATE `users` SET email_verified_at = '$this->email_verified_at',status = $this->status WHERE email = '$this->email'";
        return $this->runDML($query);
    }

    function login() {
        
        // check if email and password are the same in database
